...
function download(name, data, type = 'application/json') { const url = URL.createObjectURL(new Blob([data], { type })); const link = document.createElement('a'); link.href = url; link.download = name; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
...
document.addEventListener('click', async event => {
  ...
  else if (action === 'copy-output') await copy($('#command-output').value);
  ...
  else if (action === 'copy-blueprint') await copy(blueprint.code, 'Blueprint string copied. Paste it into Factorio’s blueprint import dialog.');
  else if (action === 'crate-blueprint' && blueprint && !blueprint.isBook && !blueprint.excluded.length) generateDialog(blueprint.entries, 'passive-provider-chest', 'Construction crate · ' + blueprint.name);
  ...
  else if (action === 'manual-grant') await copy('/starter_init_grant');
});
...
function packBlueprint(blueprint) { addRows(blueprint.entries); $('#modal').close(); location.hash = 'crates'; if (state.page === 'crates') renderPage(); notify('Blueprint materials added, with original qualities.'); }
...
try {
  ...
  explorer = globalThis.FactorioExplorer.create({ catalog, library, community, production, atlas, esc, image, svg, number, heading, searchField, empty, showModal, notify, addRows, renderPage, download });
  hydrate();
