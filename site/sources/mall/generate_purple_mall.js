...
let pods = [
    ...
    },
    {
        ...
        chainNextIndexes: [0, 1],
        inputs: [
            { item: 'iron-plate', targetX: 45, tapX: 34, bendY: 25, yOffsets: [-1, -1, undefined] },
            { item: 'iron-gear-wheel', targetX: 51, tapX: 38, bendY: 26, yOffsets: [0, undefined, 0] },
            { item: 'electronic-circuit', targetX: 44, tapX: 42, bendY: 27, yOffsets: [1, 1, 1] },
    ...
    },
    {
        ...
        chainNextIndexes: [],
        inputs: [
            { item: 'steel-plate', targetX: 59, tapX: 54, bendY: 25, yOffsets: [-1, -1, -1] },
            { item: 'copper-plate', targetX: 65, tapX: 58, bendY: 26, yOffsets: [0, 0, 0] },
            { item: 'advanced-circuit', targetX: 58, tapX: 62, bendY: 27, yOffsets: [undefined, undefined, 1] }
        ]
    },
    {
        px: 76,
        ...
        chainNextIndexes: [0],
        inputs: [
            { item: 'iron-plate', targetX: 73, tapX: 70, bendY: 25, yOffsets: [-1, -1, -1] },
            { item: 'steel-plate', targetX: 79, tapX: 74, bendY: 26, yOffsets: [undefined, undefined, 0] }
        ]
    },
    {
        px: 90,
        ...
        chainNextIndexes: [],
        inputs: [
            { item: 'steel-plate', targetX: 87, tapX: 82, bendY: 25, yOffsets: [-1, undefined, undefined] },
            { item: 'battery', targetX: 93, tapX: 86, bendY: 26, yOffsets: [0, undefined, undefined] },
            { item: 'electronic-circuit', targetX: 86, tapX: 90, bendY: 27, yOffsets: [1, undefined, 1] },
            { item: 'electric-engine-unit', targetX: 94, tapX: 94, bendY: 28, yOffsets: [1, undefined, undefined], endY: 40 },
            { item: 'advanced-circuit', targetX: 95, tapX: 98, bendY: 29, yOffsets: [undefined, 1, undefined] }
        ]
    },
    {
        px: 104,
        ...
        chainNextIndexes: [],
        inputs: [
            { item: 'iron-plate', targetX: 101, tapX: 110, bendY: 25, yOffsets: [-1] },
            { item: 'copper-cable', targetX: 107, tapX: 114, bendY: 26, yOffsets: [0] },
            { item: 'electronic-circuit', targetX: 100, tapX: 118, bendY: 27, yOffsets: [1] }
        ]
    }
];
...
for (let pod of pods) {
    ...
    if (pod.px === 90) {
        place('fast-transport-belt', 95, 41, 6);
        for (let y = 41; y <= 60; y++) {
            place('fast-transport-belt', 94, y, 4);
        }
    }
    ...
    for (let i = 0; i < pod.items.length; i++) {
        ...
        for (let input of pod.inputs) {
            if (input.isFluid) {
                ...
                if (pod.px === 90 && input.item === 'advanced-circuit') actualBeltX = 94;
        ...
        if (pod.chainNextIndexes.includes(i)) {
            place('fast-inserter', pod.px, y + 4, 4);
        }
    }
}

// Long-handed inserter branch (from inserter chest)
place('fast-inserter', 49, 38, 2); // East from inserter chest
place('assembling-machine-3', 51, 38, 2, 'long-handed-inserter');
place('fast-inserter', 51, 40, 4);
place('passive-provider-chest', 51, 41);
place('fast-inserter', 50, 37, 2); // From iron plate belt
place('fast-inserter', 51, 36, 4); // From gear belt
...
const compressed = zlib.deflateSync(JSON.stringify(blueprint));
const base64 = Buffer.from(compressed).toString('base64');
const finalString = '0' + base64;

fs.writeFileSync('./mall/purple_mall.json', jsonStr);
fs.writeFileSync('./mall/purple_mall.txt', finalString);
console.log('Blueprint generated: mall/purple_mall.json and mall/purple_mall.txt');
