...
// Input belt (plastic feed).
beltH(0, 0, 70, 2);

// Molten iron/copper pipe inputs (connect external bus here).
add('pipe-to-ground', 4.5, 9.5, { direction: 4 }); // molten iron in
add('pipe-to-ground', 16.5, 9.5, { direction: 4 }); // molten copper in

// Iron manifold to plate + gear foundries.
pipesH(10, 4, 10);

// Casting foundries (use molten inputs).
...
add('passive-provider-chest', 33.5, 13.5); // ec chest
...
add('passive-provider-chest', 43.5, 13.5); // inserter chest
...
add('passive-provider-chest', 53.5, 16.5); // fast chest
...
add('passive-provider-chest', 63.5, 13.5); // advanced chest
...
const blueprint = {
  blueprint: {
    label: 'All Inserters (Foundry Inputs)',
    icons: [
...
const outJson = './inserters/all_inserters_foundry.json';
const outTxt = './inserters/all_inserters_foundry.txt';
