...
const runFile=promisify(execFile),root=path.resolve(process.env.STARTER_TEST_ROOT||'.cache/early-game/native'),mods=path.join(root,'mods'),mod=path.join(mods,'starter_test');
...
const mapSeed=12345;
const sourceRoot=process.env.STARTER_SOURCE_ROOT||'blueprint-sources/early-game';
...
script.on_init(function()
 ...
 for _,b in pairs(builds)do
  ...
  for _,e in pairs(b.entities)do
   ...
   check(actual.position.x==e.position.x+t.dx and actual.position.y==e.position.y+t.dy,b.id..' shifted layout')
...
script.on_nth_tick(60,function()
 for _,b in pairs(builds)do local t=storage.builds[b.id]
  if game.tick==60 and b.powerNetwork then
   ...
   t.powerNetwork={bigPoles=0,mediumPoles=0,poweredConsumers=0}
   for _,e in pairs(t.entities)do
    ...
    elseif e.type=='assembling-machine' or e.type=='inserter' or e.type=='pump' then
