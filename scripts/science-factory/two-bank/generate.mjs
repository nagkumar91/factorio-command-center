...
const outputRootArg = process.env.SCIENCE_FACTORY_OUTPUT || path.join(repo, '.cache/science-factory/reproduce-electric-v2/generated');
    ...
    for (const utilization of [1, 0.9, 0.75, 0.667, 0.5]) {
      try {
        block = makeCompactRecipeBlock({
          ...
          machines: size, utilization, maxRows, inwardInputs: true, pitch: (group.recipe.category === 'smelting' || ingredients.length <= 2) ? 3 : 5, westLanePickup: 'outside'
        });
        break;
...
const mirrorBlock = original => {
  ...
  block.rows = block.rows.map(row => ({
    ...
    machine: {
      ...
      outputArms: row.machine.outputArms.map(mirrorArm)
    },
    output: row.output ? {
      ...row.output,
...
const blockBaseY = -40;
const bankPlan = JSON.parse(await fs.readFile(path.join(here, 'bank-plan.json')));
...
info.setupNotes.push('Smelt the raw ores and stone at the west end. Production runs on both sides of the bus, with direct feeds into the nearby machine rows. Each intermediate route ends after its last consumer.');
info.changes = ['Two production banks around a compact central bus; raw smelting is placed before the workshops.', 'Feeder belts enter from the bus-facing end of each machine row, eliminating long return loops.', 'One additional iron furnace and a higher gear-production limit keep all four science outputs at or above 30/min in the simultaneous benchmark.'];
...
const manifest = { ...info, blueprintSha256, entityCount: materials.entityCount, entries: materials.entries, excluded: materials.excluded, condensation, compactReport, staticConnectionAudit, bankPlan: { width: bankPlan.width, height: bankPlan.height, rows: bankPlan.rows, order: bankPlan.order, positions: bankPlan.positions }, layoutOptions: { kind: 'two-bank-direct-feed-raw-first-bus', branchGap: 8, busSpacing: 3, transportTier: 'fast', electricFurnaces: true, requestedBlockColumnGap, blockColumnGap, gridColumns, gridRowStep, blockColumnGapRejected: requestedBlockColumnGap !== blockColumnGap, blockColumnGapRejection, blockBaseX, bottomAligned: true, busEndMargin, baselineRows: initialBusRows.size, coloredRows: colors.length } };
