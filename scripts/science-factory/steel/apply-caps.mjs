...
const source=resolveRepoPath(process.argv[2]||path.join(defaultRoot,'compacted'));
const out=resolveRepoPath(process.argv[3]||path.join(defaultRoot,'capped'));
...
const capTable=JSON.parse(await fs.readFile(path.join(path.dirname(new URL(import.meta.url).pathname),'headroom-caps.json')));
...
for(const [recipe,target] of Object.entries(targets)){
 if(process.env.SKIP_STEEL_CAP==='1' && recipe==='steel-plate') continue;
