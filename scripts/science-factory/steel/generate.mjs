...
import { encodeBlueprint, blueprintMaterials } from '../../../scripts/blueprints.mjs';
import { addInputDisplays } from '../../../scripts/starter-input-displays.mjs';
...
const outputRootArg = process.env.STEEL_GENERATOR_OUT || path.join(repo, '.cache/science-factory/reproduce-steel/generated');
...
const counts = {
  ...
  'iron-plate': 23, 'copper-plate': 10, 'steel-plate': 6, 'stone-brick': 5,
  ...
  'transport-belt': 1, 'engine-unit': 8, 'advanced-circuit': 8,
...
// Steel furnaces are unlocked by Advanced material processing.  The electric
// candidate this scratch copy came from required AM2, but retaining that
// unlock here would overstate the research needed for the steel variant.
const required = new Set(['automation-2', 'advanced-material-processing', 'electric-energy-distribution-1', 'fast-inserter', 'logistics-2', 'circuit-network']);
...
for (const id of ['fast-transport-belt', 'fast-underground-belt', 'fast-splitter', 'big-electric-pole', 'medium-electric-pole', 'display-panel', 'steel-furnace']) {
  const options = unlocks.get(id) || [];
  if (options.length) required.add([...options].sort((a, b) => closure(a).size - closure(b).size || a.localeCompare(b))[0]);
}
    ...
    for (const utilization of [1, 0.9, 0.75, 0.667, 0.5]) {
      try {
        block = makeCompactRecipeBlock({
          ...
          recipe: blockRecipe,
          machine: group.recipe.category === 'smelting' ? 'steel-furnace' : 'assembling-machine-2',
          machines: size, utilization, maxRows, pitch: group.recipe.category === 'smelting' ? 4 : 5, westLanePickup: 'outside'
        });
        break;
...
const itemOrder = [...new Set(['solid-fuel', ...itemRawInputs, ...[...itemSet].filter(item => item !== 'solid-fuel' && !itemRawInputs.includes(item)).sort()])];
...
const externalOutputPorts = [
  ...
  { id: 'oil-solid-fuel', material: 'solid-fuel', grid: { x: 58, y: -9 }, direction: 8, routeMode: 'south' }
];
...
for (const entity of entities) {
  if (entity.name === 'steel-furnace' && entity.recipe) {
...
const adapterPath = path.join(repo, 'blueprint-sources/science-factories/components/oil-steel-bus.json');
...
const info = {
  id: 'science-four-pack-30-steel', file: 'science-four-pack-30-steel.txt', name: 'Steel science factory · 30/min each',
  ...
  fuelPolicy: { furnaceFuel: 'solid-fuel', furnaceEntities: ['steel-furnace'], coalAllowedFor: ['grenade', 'plastic-bar'], internalProduction: true, externalFuelInjection: false },
  ...
  setupNotes: ['Feed iron ore, copper ore, coal, stone, water and crude oil at the six labeled west-side ports.', 'The four science output chests are on the east edge.', 'Steel furnaces consume internally produced solid fuel from the light-oil branch; coal is reserved for grenades and plastic.', 'Fast transport belts, fast underground belts and fast splitters are used; no robots, modules, beacons or quality are included.'],
  changes: ['Pure item recipes use deterministic compact blocks and one eastbound bus per material.', 'Steel furnaces replace electric furnaces and the oil adapter solid-fuel output is routed to every steel-furnace block.']
};
