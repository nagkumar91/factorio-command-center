...
const source=resolveRepoPath(process.argv[2]||'.cache/science-factory/reproduce-electric-v3/generated/blueprint-sources/science-factories');
const out=resolveRepoPath(process.argv[3]||'.cache/science-factory/reproduce-electric-v3/capped');
...
for(const port of info.ports){const d={north:[0,-1],east:[1,0],south:[0,1],west:[-1,0]}[port.externalSide];if(d)occupied.add(key(Math.floor(port.x+d[0]),Math.floor(port.y+d[1])));}
