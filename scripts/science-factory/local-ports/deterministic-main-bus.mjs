...
function localBlockCells(block) {
  ...
  for (const row of block.rows || []) {
    ...
    for (const arm of [...(row.machine.inputArms || []), ...(row.machine.outputArms || []), ...(row.machine.outputTransferArms || [])]) claim(arm.grid, `inserter:${arm.entity}`);
  }
  return cells;
}
...
function crossRows(state, { from, toY, direction, material, routeId, rows, x }) {
  ...
  for (const row of crossed) {
    ...
    if (last && Math.abs(row - last[0]) + 2 <= 7) last.push(row);
    else groups.push([row]);
  }
  for (const group of groups) {
    const row = group[0], lastRow = group.at(-1);
    const input = { x: column, y: ascending ? row - 1 : row + 1 };
    const output = { x: column, y: ascending ? lastRow + 1 : lastRow - 1 };
    // A southbound branch may begin directly on the first tunnel input
    // (bus.y + 2); a northbound branch reaches its first input from the
    // immediately adjacent ordinary belt.  Do not ask addStraight to walk
    // backwards or place an ordinary belt on an underground endpoint.
    const pre = { x: column, y: ascending ? input.y - 1 : input.y + 1 };
    if (!eq(current, input) && !eq(current, pre)) {
...
function routeInput(state, record, port, bus, branchIndex, allRows, options) {
  ...
  const southBank = record.side === 'south';
  const branchDirection = southBank ? SOUTH : NORTH;
  ...
  const first = { x: column, y: bus.y + (southBank ? 1 : -1) };
  ...
  addBelt(state, first, branchDirection, { material: port.material, route: routeId, role: 'splitter-output' });
  if (record.block?.inwardInputs) {
    if (column !== port.grid.x) fail('direct inward branch must align with its port');
    const crossing = crossRows(state, { from: first, toY: port.grid.y, direction: branchDirection, material: port.material, routeId, rows: allRows, x: column });
    turnOwnedBelt(state, port.grid, port.direction, routeId);
    const target = state.beltsByCell.get(key(port.grid.x, port.grid.y));
    if (!target || target.name !== BELT) fail('direct inward target must be an ordinary belt');
  ...
  const approachY = southBank
    ? port.side === 'west' && port.lane === 0
      ? port.grid.y
      : port.side === 'west'
        ? port.grid.y + 1
        : port.grid.y + 2
    : port.side === 'west' && port.lane === 0
      ...
      : port.side === 'west'
        ? port.grid.y - 1
        : port.grid.y - 2;
  const crossing = crossRows(state, { from: first, toY: approachY, direction: branchDirection, material: port.material, routeId, rows: allRows, x: column });
  ...
  if (port.side === 'west' && port.lane === 0) {
    ...
    turnOwnedBelt(state, { x: port.grid.x, y: approach.y }, southBank ? NORTH : SOUTH, routeId);
    ...
    addHorizontalAcrossColumns(state, approach, { x: port.grid.x, y: approach.y }, EAST, port.material, routeId);
    addVertical(state, { x: port.grid.x, y: approach.y }, port.grid, southBank ? NORTH : SOUTH, port.material, routeId);
  }
  const end = state.addRouteNode(port.grid, port.material, routeId, 'input-target', port.direction);
  state.routes.push({ id: routeId, kind: 'input-fanout', blockId: record.id, material: port.material, source: { splitter: splitter.entity_number, bus: { ...bus } }, target: end, branchColumn: column, crossedRows: crossing.crossed, tunnelIds: crossing.tunnelIds });
}
...
function routeOutput(state, record, bus, branchIndex, allRows, options) {
  ...
  state.currentRoute = routeId;
  const southBank = record.side === 'south';
  const branchDirection = southBank ? NORTH : SOUTH;
  ...
  addBelt(state, { x: turnX, y: source.y }, branchDirection, { material: record.outputPort.material, route: routeId, role: 'producer-turn' });
  const crossing = crossRows(state, { from: { x: turnX, y: source.y }, toY: bus.y, direction: branchDirection, material: record.outputPort.material, routeId, rows: allRows, x: branchColumn });
  const final = { x: crossing.column, y: bus.y };
...
function routeLocalOutput(state, record) {
  const routeId = `local-output:${record.id}`;
  state.currentRoute = routeId;
  const source = record.outputPort.grid;
  addBelt(state, source, EAST, { material: record.outputPort.material, route: routeId, role: 'producer-port' });
  const target = { x: source.x + 1, y: source.y };
  addBelt(state, target, EAST, { material: record.outputPort.material, route: routeId, role: 'local-output' });
  state.routes.push({ id: routeId, kind: 'local-output', blockId: record.id, material: record.outputPort.material,
    source: { ...source }, target: { grid: target, role: 'local-output', direction: EAST },
    branchColumn: target.x, crossedRows: [], tunnelIds: [] });
}
...
export function auditDeterministicBus(result) {
  ...
  for (const route of result.routes) {
    if (route.kind !== 'local-output' && !rows.has(route.material)) fail(`${route.id} has no material bus`);
    ...
    if (route.kind === 'local-output' && route.target.role !== 'local-output') fail(`${route.id} has no local output target`);
  }
 ...
 */
export function routeCompactBlocks({ blocks = [], placements = {}, busRows, busStartX = -64, busEndX = 128, branchGap = 8, inputBranchStart, outputBranchStart, externalInputs = [], externalOutputs = [], trimBusRanges = false, externalBusRanges = {}, outputMergeSplitters = false, inputBranchWestOffsets = { lane0: 8, lane1: 10 }, inputBranchEastOffset = 8, localOutputMaterials = [] } = {}) {
  ...
  const busMinY = Math.min(...rows.map(row => row.y));
  const busMaxY = Math.max(...rows.map(row => row.y));
  for (const record of state.blocks) {
    const top = record.placement.y + record.block.footprint.top;
    const bottom = record.placement.y + record.block.footprint.bottom;
    if (bottom <= busMinY - 3) record.side = 'north';
    else if (top >= busMaxY + 3) record.side = 'south';
    else fail(`${record.id} must be wholly north or south of bus strip (${top}..${bottom}, rows ${busMinY}..${busMaxY})`);
  }
  ...
  const localOutputSet = new Set(localOutputMaterials);
  ...
  // Each block bank has its own three-cell clearance to the central bus
  // strip.  North and south routes are mirrored; both share the same
  // eastbound material rows.
  ...
  const inputBranchColumn = (record, port) => {
    if (record.block.inwardInputs) return port.grid.x;
  ...
  for (const record of ordered) for (const port of record.inputPorts) {
    ...
    list.push({ x: inputBranchColumn(record, port), kind: 'fanout', blockId: record.id, portIndex: port.index, side: record.side });
    inputJunctionsByMaterial.set(port.material, list);
  }
  for (const record of externalInputRecords) {
    const port = record.inputPort;
    const list = inputJunctionsByMaterial.get(port.material) || [];
    list.push({ x: port.branchColumn, kind: 'fanout', blockId: record.id, portIndex: 0, side: record.side });
    inputJunctionsByMaterial.set(port.material, list);
  }
  const rangePlans = [];
  ...
  for (const row of rows) {
    const materialOutputRecords = outputRecords.filter(record => record.outputPort.material === row.material && !localOutputSet.has(record.outputPort.material));
    ...
    for (const junction of junctions) {
      if (junction.kind === 'fanout' || outputMergeSplitters) {
        const splitterGrid = junction.kind === 'fanout'
          ? { x: junction.x - 1, y: junction.side === 'south' ? row.y : row.y - 1 }
          : { x: junction.x + 1, y: row.y };
  ...
  for (let i = 0; i < outputRecords.length; i++) {
    ...
    if (localOutputSet.has(record.outputPort.material) && record.id.startsWith('recipe-block-')) {
      routeLocalOutput(state, record);
    } else {
