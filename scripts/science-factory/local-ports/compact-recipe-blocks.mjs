...
export function makeCompactRecipeBlock({
  ...
  const geometry = machineGeometry(machine);
  const smelting = machine === 'steel-furnace' || machine === 'electric-furnace';
  // Three-ingredient blocks share C/result lanes, with filtered result
  // extraction and a straight inlet guide preventing automatic belt curves.
  const sideOutputWithC = normalized.ingredients.length === 3 && !smelting;
  ...
  // Keep the C neck straight: without a rear belt the game curves it into
  // the side feed and fills both lanes, blocking the result inserter lane.
  if (sideOutputWithC && inwardInputs) addVerticalBelt(state, {
    id: 'c-lane-guide', gridX: 3, gridY: inputNeckY + 1,
    materials: [eastAssignment.material], laneMaterials: {near: eastAssignment.material},
    role: 'input-c-lane-guide', row: null, laneRate: {}, direction: inputDirection
  });
...
export function checkCompactRecipeBlock(block) {
  ...
  for (const input of block.inputs) {
    ...
    if (input.side === 'west') {
      ...
      if (block.inwardInputs && !block.belts.some(belt => belt.role === 'input-c-lane-guide' && belt.grid.x === neck.grid.x && belt.grid.y === neck.grid.y + 1 && belt.direction === DIRECTIONS.north)) fail(`C route target for ${input.material} would curve and fill both lanes`);
    } else {
