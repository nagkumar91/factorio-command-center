...
export function makePowerLayout({products,nodes,raw,technologies,attempt=0,assembler='assembling-machine-2',intermediateAssembler='assembling-machine-2'}){
 const finalProducts=new Set(products);
 const entities=[],occupied=new Map(),reserved=new Map(),nets=new Map(),ports=[],poles=[];
 const pitch=attempt<12?10:attempt<64?12:attempt<192?16:20;
 ...
 function add(name,x,y,props={},w=1,h=1){
  ...
  for(let tx=Math.round(x-w/2);tx<x+w/2-.01;tx++)for(let ty=Math.round(y-h/2);ty<y+h/2-.01;ty++){
   const k=key(tx,ty);if(occupied.has(k)){const old=occupied.get(k).entity;throw Error('Overlap '+name+' '+k+' with '+old.name+'#'+old.entity_number);}occupied.set(k,{entity,fluid:props.tags?.fluid});
  }
 ...
 nodes.forEach((node,nodeIndex)=>{
  ...
  const machine=category==='smelting'?'electric-furnace':category==='oil-processing'?'oil-refinery':category==='chemistry-or-cryogenics'?'chemical-plant':(node.item&&finalProducts.has(node.item)?assembler:intermediateAssembler);
  if(!machine)throw Error('Unsupported machine for '+r.name);
  const proto=raw.furnace[machine]||raw['assembling-machine'][machine];
  const required={
   'electric-furnace':'advanced-material-processing-2',
   'assembling-machine-2':'automation-2',
   'assembling-machine-3':'automation-3',
  ...
  if(category==='smelting'){
   ...
   if(finalProducts.has(node.item)){add('inserter',cx+1.5,cy-.5,{direction:12});finalEntities.set(node.item,add('wooden-chest',cx+2.5,cy-.5,{bar:1}));}
   // Electric furnaces occupy 3x3 tiles; leave a full tile outside their
   // east edge for the intermediate output inserter.
   else inserter(node.item,cx+2,cy-1,12,true);
   // Electric furnaces are 3x3, so place their local poles one tile farther
   // out than the old steel-furnace pattern.
   pole(cx-3,cy-3);pole(cx+2,cy+2);
  }else{
   const radius=(w-1)/2;
   ...
   if(!raw.fluid[node.item]){
    if(finalProducts.has(node.item)){add('inserter',cx+radius+1.5,cy+.5,{direction:12});finalEntities.set(node.item,add('wooden-chest',cx+radius+2.5,cy+.5,{bar:1}));}
 ...
 function route(start,goal,net,previous){
  ...
  throw Error('No route '+net.item+' from '+key(start.x,start.y)+' to '+key(goal.x,goal.y)+' targets='+net.targets.map(t=>key(t.x,t.y)).join('|'));
 }
 function placeNet(net){
  // Fluid consumers are better served by independent branches from the
  // producer/port.  A serial branch made the next target start on the first
  // machine's pipe connection, where the surrounding machine footprint often
  // leaves no legal turn.  Keep the branch source shared, but route each
  // target from it and save the source pipe only once.
  if(net.fluid){
   const targets=net.targets.slice().sort((a,b)=>distance(net.source,a)-distance(net.source,b));
   let first=true;
   for(const goal of targets){
    const segment=route(net.source,goal,net);
    const local=[{...net.source,span:1},...segment];
    for(let i=1;i<local.length;i++){
     const prev=local[i-1],step=local[i];
     if(step.span>1){prev.tunnelStart=true;prev.direction=step.dir;step.tunnelEnd=true;step.direction=step.dir;prev.tunnelKey=step.tunnelKey=entities.length+':'+i;}
     else if(!prev.tunnelEnd)prev.direction=step.dir;
    }
    for(const p of local)if(first||p!==local[0])occupied.set(key(p.x,p.y),{routing:true,fluid:net.item,routingTunnel:p.tunnelStart?p.direction:p.tunnelEnd?(p.direction+8)%16:undefined});
    for(const p of local)if(first||p!==local[0])occupied.delete(key(p.x,p.y));
    for(const p of local){
     if(!first&&p===local[0])continue;
     const tunnel=p.tunnelStart||p.tunnelEnd;
     const props={...(tunnel?{direction:p.tunnelStart?(p.direction+8)%16:p.direction}:{}),tags:{fluid:net.item,tunnel:p.tunnelKey}};
     add(tunnel?'pipe-to-ground':'pipe',p.x+.5,p.y+.5,props);
    }
    first=false;
   }
   return;
 ...
 for(const net of [...nets.values()].sort((a,b)=>b.targets.length-a.targets.length)){
  ...
  for(const output of outputs){
   ...
   targets.sort((a,b)=>distance(output,a)-distance(output,b));
 ...
 for(const product of products){
  const finalEntity=finalEntities.get(product);if(!finalEntity)throw Error('Missing final output '+product);
  ports.push({kind:'output',label:'OUT · '+product,entity:finalEntity.entity_number,items:[product],x:finalEntity.position.x,y:finalEntity.position.y});
 }
 ...
 return {blueprint:{item:'blueprint',version:562949958467584,entities,wires,icons:products.map((name,index)=>({index:index+1,signal:{type:'item',name}}))},ports,machines:nodes.length,pitch};
}
