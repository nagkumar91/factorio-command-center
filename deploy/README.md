...
The analytics scripts run after the application renders. Events are batched with a one-second delay and sent without awaiting the collector. Browser tests deliberately stall the analytics script and collector to verify that search and navigation stay usable.

## Hourly checks and Codex repair

The OpenClaw agent `factorio-site-repair` uses the Pi's Codex runtime (`openai/gpt-5.6-luna`); the hourly publisher invokes the signed-in Codex CLI. Its workspace is `~/clawd/workspaces/factorio-site-repair`, with a dedicated GitHub checkout in `source/`. The source's `ops/REPAIR_AGENT.md` and workspace `AGENTS.md` describe the owner's standing authorization to fix, test, and republish this site. This agent alone has `tools.exec.host=gateway`, `tools.exec.mode=full`, and a gateway execution approval policy with `security=full` and `ask=off`. Both the requested agent policy and execution-host policy must agree for Codex to work unattended. Preserve every other agent's policy and the global defaults.
